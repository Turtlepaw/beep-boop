...
const CommandData = new TriviaSubcommandBuilder(
  "start",
  "❓ Play some trivia."
);
    ...
    super({
      ...
      Subcommands: [
        new SlashCommandSubcommandBuilder()
          ...
          .addUserOption((e) =>
            e
              .setName("member")
              .setDescription(
                "Play with a friend, select the member to invite below."
              )
          ),
        new SlashCommandSubcommandGroupBuilder()
          ...
          //   .addSubcommand(
          //     CommandData.toBuilder() as unknown as SlashCommandSubcommandBuilder
          //   )
          .addSubcommand((subcmd) =>
            subcmd
              .setName("end")
              .setDescription(
                "Ends the current game. Must be the owner of the game or a moderator."
              )
          ),
      ],
    });
    ...
    if (SubcomamndName == Subcommands.RockPaperScissors) {
      ...
      if (Member != null) {
        if (!(Member instanceof GuildMember)) {
          SendError(
            interaction,
            "CANNOT PROCESS REQUEST: INVALID GUILD MEMBER"
          );
          return;
        }
        new MultiplayerRockPaperScissors()
          .setMember(Member as unknown as any)
          .StartGame(interaction as unknown as any);
      } else {
        new RockPaperScissors().StartGame(interaction as unknown as any);
      }
    } else if (SubcomamndName == Subcommands.Trivia) {
      const Options = CommandData.getOptions(interaction);
      const game = trivia.createGame(interaction as unknown as any);
