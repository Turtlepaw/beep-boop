...
export function GetEmoji(client: Client, module: Modules) {
  ...
  const EmojiId = /<(a)?:\w+:(\d{19})>/gi.exec(Information.Icon.toString())[2];
  ...
  async ExecuteInteraction(
    ...
    const SelectorComponents = {
      ...
      RemoveRole: new StringSelector()
        ...
        .AddOptions(
          ...(await Promise.all(
            Configuration.Verification.Roles.map(async (e) => {
    ...
    const Components = () => [
      new ButtonBuilder()
        ...
        .setStyle(ButtonBoolean(Verification)),
      new ButtonBuilder()
...
${Icons.StemEnd} Roles: ${
              Roles == null || Roles.length == 0
                ? "None"
                : Roles.map((e) => roleMention(e)).join(" ")
            }`,
          },
        ]);
    ...
    Collector.on("collect", async (button) => {
      ...
      if (button.isButton() && button.customId == Id.ToggleModule) {
        ...
        if (Roles == null || Roles.length == 0) {
          await FriendlyInteractionError(
            button,
