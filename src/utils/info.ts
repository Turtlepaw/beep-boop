...
export async function MemberInformation(
  ...
  const Flags = {
    ...
    VerifiedBot:
      Icons.VerifiedBotCheck.toString() + Icons.VerifiedBotText.toString(),
    Bot: Icons.Bot,
  ...
  await interaction.reply({
    embeds: [
      new Embed(interaction)
        ...
        .addFields([
          {
            ...
            )}${Or}${time(Member.joinedAt, TimestampStyles.RelativeTime)}`,
            inline: false,
          },
          {
            ...
            value: `${time(
              User.createdAt,
              TimestampStyles.LongDateTime
            )}${Or}${time(User.createdAt, TimestampStyles.RelativeTime)}`,
            inline: false,
          },
          {
            ...
            value:
              User.hexAccentColor != null
                ? inlineCode(User.hexAccentColor)
                : "No accent color",
          },
          {
            ...
            value:
              MemberRoles.size >= 1
                ? MemberRoles.map((e) => e.toString()).join(" ")
                : "No roles",
            inline: false,
          },
          {
            ...
            value:
              Member.id == interaction.user.id && interaction.locale != null
          ...
          },
          {
            ...
            value:
              flags.length >= 1
                ? flags
                    .map((e) => (Flags[e] != null ? Flags[e] : inlineCode(e)))
                    .join(" ")
                : "No badges",
            inline: false,
          },
        ...
        .setColor(
          Member.displayColor == 0 ? Colors.Transparent : Member.displayHexColor
        )
        .setFooter({
          text: `ID: ${Member.id}`,
        })
...
export async function RoleInformation(
  interaction: RepliableInteraction,
  targetRole: GuildRole | APIRole,
  hidden = false
) {
  const { guild } = interaction;
  if (guild == null)
        ...
        .addFields([
          {
            ...
            value: `${time(
              Role.createdAt,
              TimestampStyles.LongDateTime
            )}${Or}${time(Role.createdAt, TimestampStyles.RelativeTime)}`,
            inline: false,
          },
          {
          ...
          },
          {
            ...
            value:
              Role.hexColor != null ? inlineCode(Role.hexColor) : "No color",
            inline: false,
          },
          {
            name: `${Icons.Configure} Permissions`,
            value: Role.permissions.has(PermissionFlagsBits.Administrator)
              ? inlineCode("Administrator")
              : Role.permissions.toArray().length >= 1
              ? Role.permissions
                  .toArray()
                  .map((e) => inlineCode(e.toString()))
                  .join(" \u200b".repeat(3))
              : "No permissions",
            inline: false,
          },
          {
            ...
            value:
              Flags.length >= 1
                ? Object.entries(RoleFlags)
                    .filter((e) => Flags.includes(e[1] as RoleFlags))
                    .map(([k, v]) => `${RoleFlagIcons[v]} (${k.toLowerCase()})`)
                    .join(" \u200b".repeat(3))
                : "No flags",
            inline: false,
          },
...
export async function GuildInformation(
  interaction: RepliableInteraction,
  targetGuild: DiscordGuild,
  hidden = false,
  withComponents: ActionRowBuilder<ButtonBuilder>[] = []
) {
  const { guild } = interaction;
  if (guild == null)
    return interaction.reply({
        ...
        .addFields([
          ...
          },
          {
            ...
            value: `${time(
              Guild.createdAt,
              TimestampStyles.LongDateTime
            )}${Or}${time(Guild.createdAt, TimestampStyles.RelativeTime)}`,
            inline: false,
          },
          {
            name: `${Icons.Flag} Roles`,
          ...
          },
          {
            ...
            value: `${Icons.Folder} (categories) ${
              ...
              Guild.channels.cache.filter(
                (e) =>
                  e.type == ChannelType.GuildText ||
                  e.type == ChannelType.GuildAnnouncement
              ).size
            } | ${Icons.Voice} (voice) ${
              Guild.channels.cache.filter(
                (e) =>
                  e.type == ChannelType.GuildVoice ||
                  e.type == ChannelType.GuildStageVoice
              ).size
            } `,
          },
        ])
