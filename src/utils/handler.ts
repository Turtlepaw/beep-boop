...
async function StartEventService(client: Client) {
  try {
    ...
    const Files = klawSync(`${BaseDirectory}/events`, {
      nodir: true,
      traverseAll: true,
...
export async function HandleBotPermissions(
  ...
  permisisons: PermissionResolvable[]
) {
  ...
  if (
    ...
    await InteractionError({
      createError: false,
      interaction,
      message: `The bot doesn't have the correct permissions to run this command. Missing: ${Missing.map(
        (e) => inlineCode(e)
      ).join(" ")}`,
    });
    return false;
...
async function StartButtonService(client: Client) {
  try {
    ...
    const Files = klawSync(`${BaseDirectory}/buttons`, {
      nodir: true,
      traverseAll: true,
      filter: (f) => f.path.endsWith(".js"),
    });
    ...
    client.on(ClientEvents.InteractionCreate, async (interaction) => {
      if (interaction.isButton()) {
        ...
        if (interaction.guild != null) {
          if (
            Button.SomePermissions.length >= 1 &&
            !interaction.memberPermissions.any(Button.SomePermissions)
          ) {
            interaction.reply(ButtonPermissionsMessage);
            return;
          }
          if (
            Button.RequiredPermissions.length >= 1 &&
            !interaction.memberPermissions.has(Button.RequiredPermissions)
          ) {
            interaction.reply(ButtonPermissionsMessage);
            return;
          }
        ...
        if (
          (await HandleBotPermissions(interaction, Button.ClientPermissions)) ==
          false
        )
          return;
...
async function StartSelectMenuService(client: Client) {
  try {
    //Find the button files
    const Files = klawSync(`${BaseDirectory}/select_menus`, {
      nodir: true,
      traverseAll: true,
      filter: (f) => f.path.endsWith(".js"),
    });
    ...
    client.on(ClientEvents.InteractionCreate, async (interaction) => {
      if (interaction.isAnySelectMenu()) {
        ...
        const SelectOption = SelectOptions.find((e) =>
        ...
        if (interaction.guild != null) {
          if (
            SelectOption.SomePermissions.length >= 1 &&
            !interaction.memberPermissions.any(SelectOption.SomePermissions)
          ) {
            interaction.reply(ButtonPermissionsMessage);
            return;
          }
          if (
            SelectOption.RequiredPermissions.length >= 1 &&
            !interaction.memberPermissions.has(SelectOption.RequiredPermissions)
          ) {
            interaction.reply(ButtonPermissionsMessage);
            return;
          }
        ...
        if (
          (await HandleBotPermissions(
            interaction,
            SelectOption.ClientPermissions
          )) == false
        )
          return;
  ...
  try {
    ...
    client.on(ClientEvents.InteractionCreate, async (interaction) => {
      if (interaction.isContextMenuCommand()) {
        ...
        if (interaction.guild != null) {
          if (
            ContextMenu.SomePermissions.length >= 1 &&
            !interaction.memberPermissions.any(ContextMenu.SomePermissions)
          ) {
            interaction.reply(InputPermissionsMessage);
            return;
          }
          if (
            ContextMenu.RequiredPermissions.length >= 1 &&
            !interaction.memberPermissions.has(ContextMenu.RequiredPermissions)
          ) {
            interaction.reply(InputPermissionsMessage);
            return;
          }
    ...
    client.on(ClientEvents.InteractionCreate, async (interaction) => {
      if (interaction.isAutocomplete()) {
        ...
        try {
          ...
          // Log it in the logger
          Logger.error(
            `Error executing ${interaction.commandName}'s autocomplete:`,
            e
          );
        }
      }
  ...
  try {
    ...
    client.on(ClientEvents.InteractionCreate, async (interaction) => {
      if (interaction.isChatInputCommand()) {
        ...
        if (interaction.guild != null) {
          if (
            command.SomePermissions.length >= 1 &&
            !interaction.memberPermissions.any(command.SomePermissions)
          ) {
            interaction.reply(InputPermissionsMessage);
            return;
          }
          if (
            command.RequiredPermissions.length >= 1 &&
