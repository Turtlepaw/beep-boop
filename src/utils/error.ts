...
export async function SendError(
  ...
  console.warn(
    "The SendError function is deprecated, use InteractionError instead"
  );
...
export async function FriendlyInteractionError(
  interaction: Interaction,
  errorMessage: string,
  ephemeral = true
) {
...
export interface InteractionErrorOptions {
  ...
  message?: string;
  icon?: /* Icons is now an object */ Icon;
  ephemeral?: boolean;
  error?: string;
  createError?: boolean;
}
...
export async function InteractionError(options: InteractionErrorOptions) {
  const { icon, interaction, message, ephemeral, error, createError } =
    Object.assign(DefaultInteractionErrorOptions, options);
  const Interaction = new InteractionReplyManager(interaction);
  const ErrorDB = createError
    ? await interaction.client.Storage.Errors.Create({
    ...
    embeds: createError
      ? [
          await new Embed(interaction)
            .setDescription(
              `Show this ID when reporting this bug: ${inlineCode(
                //@ts-expect-error this exists
                ErrorDB.Error
              )}`
            )
