...
export function EmbedModal(
  ...
  const Fields = {
    ...
    Description: new TextInputBuilder()
      ...
      .setPlaceholder(
        "Here's the embed's description, you can write up to 4000 characters"
      ),
  ...
  if (Verifiers.String(Embed?.description))
    Fields.Description.setValue(Embed?.description);
  if (Verifiers.String(Embed?.footer?.text))
    Fields.FooterText.setValue(Embed?.footer?.text);
  if (Verifiers.String(Embed?.hexColor))
    Fields.Color.setValue(Embed?.hexColor || "#5865f2");

  return new ModalBuilder()
    .setCustomId(CustomId)
    .setTitle("Configuring Embed")
    .addComponents(
      new ActionRowBuilder<TextInputBuilder>().addComponents(Fields.Title),
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        Fields.Description
      ),
      new ActionRowBuilder<TextInputBuilder>().addComponents(Fields.FooterText),
      new ActionRowBuilder<TextInputBuilder>().addComponents(Fields.Color)
    );
}
...
export function EmbedFrom(Modal: ModalSubmitInteraction) {
  ...
  if (Verifiers.String(Fields.Description))
    Embed.setDescription(Fields.Description);
  ...
  if (Verifiers.String(Fields.Color) && Verifiers.isHexColor(Fields.Color))
...
export type SelectorConfiguration<T extends AnySelectMenuBuilder> = (
  SelectMenu: T
...
> {
  ...
  public toActionRow(): ActionRowBuilder<T> {
    return new ActionRowBuilder<T>().addComponents(this.toBuilder());
  }
  ...
  public async CollectComponents(
    message: Message,
    interaction?: Interaction
  ): Promise<Awaited<I>> {
    const CustomIds = [this.CustomId];
    if (interaction != null)
      message.components.forEach((e) =>
        e.components.forEach((e) => CustomIds.push(e.customId))
      );
...
export class StringSelector extends Selector<
  StringSelectMenuBuilder,
  StringSelectMenuInteraction
> {
  ...
  protected toInternalBuilder(): StringSelectMenuBuilder {
    if (this?.CustomId == null || typeof this.CustomId != "string")
      throw new Error("CustomId must be a string and cannot be left null");
...
export class ChannelSelector extends Selector<
  ChannelSelectMenuBuilder,
  ChannelSelectMenuInteraction
> {
...
export class RoleSelector extends Selector<
  RoleSelectMenuBuilder,
  RoleSelectMenuInteraction
> {
  constructor() {
  ...
  protected toInternalBuilder(): RoleSelectMenuBuilder {
    if (this?.CustomId == null || typeof this.CustomId != "string")
      throw new Error("CustomId must be a string and cannot be left null");
    const Builder = new RoleSelectMenuBuilder().setCustomId(this.CustomId);
    if (this?.Configuration != null) this.Configuration(Builder);
    return Builder;
  }
}
...
export function ChannelSelectMenu(
  ...
  Configuration?: (selectMenu: ChannelSelectMenuBuilder) => unknown
) {
...
export function MessageBuilderModal(
  CustomId = "MESSAGE_BUILDER_MODAL",
  FieldId = "MESSAGE_CONTENT_FIELD",
  Message: GuildMessage
) {
  ...
  if (Verifiers.String(Message?.content))
    ContentField.setValue(Message.content);
  return new ModalBuilder()
    .setCustomId(CustomId)
...
export function ButtonBuilderModal(
  CustomId = "BUTTON_BUILDER_MODAL",
  Fields: ButtonFields,
  ButtonType: ButtonStyle = ButtonStyle.Primary
) {
  const Components = [
    new ActionRowBuilder<TextInputBuilder>().addComponents(
