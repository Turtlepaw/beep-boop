...
export { ButtonTypes as CustomButtonType };
...
export default class AddButton extends ContextMenu {
  ...
  public async ExecuteContextMenu(
    ...
    const Webhook = await FindWebhook(
      ...
      client
    ); //client.Storage.Get(`custom_${Target.id}`);

    if (Webhook == null)
      return FriendlyInteractionError(
        interaction,
    ...
    const LinkButtonModal = ButtonBuilderModal(
      ModalId.Modal,
      Ids,
      ButtonStyle.Link
    );
    ...
    console.log(Icons.Flag, Icons.Link, Icons.Folder, Icons.Shield);
    ...
    const RoleSelectorComponent = new RoleSelector()
      .Configure((builder) => builder.setPlaceholder("Select a role"))
      .SetCustomId(Id.RoleSelector)
      .toActionRow();

    const Message = await interaction.reply({
      content: `${Icons.Tag} Select a button type below`,
      components: [ButtonTypeSelector],
      ephemeral: true,
      fetchReply: true,
    });
    ...
    const isVerificationButton =
      ButtonType.values[0] == ButtonTypes.VerificationButton;
    let StyleReplyMessage: MessageComponentInteraction = ButtonType;
    let ButtonCustomId: string = isTicketButton
      ? `OPEN_TICKET_${generateId(5)}`
      : `VERIFY_USER`;
    ...
    if (isRoleButton) {
      ...
      SelectedRole = (
        StyleReplyMessage as RoleSelectMenuInteraction
      ).roles.first();

      // ~T~a~s~k~ (completed): Return error if the role is already there
      let isError = false;
      Target.components.forEach((row) => {
        const result = ArrayUtils.AdvancedSearch<APIButtonComponent>(
          row.components.filter(
            (e) => e.type == ComponentType.Button
    ...
    const ButtonReply =
      StyleReplyMessage != null ? StyleReplyMessage : ButtonType;
    ...
    if (Value.customId == Id.CustomValues) {
      Value.showModal(isLinkButton ? LinkButtonModal : ButtonModal);
    ...
    if (isTicketButton || isVerificationButton || isRoleButton) {
      ...
      const ButtonStyles = new ActionRowBuilder<ButtonBuilder>().addComponents(
      ...
      console.log(ReplyMessage.type, ReplyTo.isButton());
    ...
    await Webhook.editMessage(Target.id, {
      components: [
        new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(
          ...(Target.components[0]?.components != null
            ? Target.components[0].components.map((e) => {
