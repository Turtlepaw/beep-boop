import { Box, Button, Center, Code, IconButton, Spinner, VStack, useClipboard } from '@chakra-ui/react';
...
import { ChevronDown20Filled as DownIcon, ChevronUp20Filled as UpIcon } from "@fluentui/react-icons";
    ...
    return (
        <div>
            <AutoCenter className='text-center' pb={5}>
                ...
                {guilds.isLoading && <Center><Spinner /></Center>}
                {guilds.error &&
                    <VStack pt={2}>
                        <ErrorMessage>Something went wrong, try again in a few minutes.</ErrorMessage>
                        <Box cursor="pointer" onClick={() => expand(!expanded)} className='hover:opacity-80'>
                            {expanded ? "Less" : "More"} Details {expanded ? <UpIcon /> : <DownIcon />}
                        </Box>
                        {expanded && <Box pb={5} pt={3}>
                            <Code p={5} mx={20} overflow="hidden" backgroundColor="#2f3136" textColor="white" borderColor="#202225" borderWidth={1.3} borderRadius="md" lang='js'>
                ...
                {guilds.data && <div className='FlexContainer2'>
                    {guilds.data.map(e => {
                        if (e == null) return;
                        return (
