...
const token =
  `${process.env.API_TOKEN}` || "Bearer api_token_1490ujdsifh9124yf";
  ...
  return [
    {
      ...
      Author: {
        Avatar:
          "https://cdn.discordapp.com/avatars/820465204411236362/aa4ece5f0f241fad5e3e554e5ef63887.webp",
        Tag: "Turtlepaw#0001",
        Username: "Turtlepaw",
      },
...
export async function Transcript(id: string) {
  return CallAPI<TicketMessage[]>(
    CreateRoute(Routes.Transcripts.replace(":id", id) as Routes)
  );
}

export async function TicketData(id: string) {
  return CallAPI<ApiTicket>(
    CreateRoute(Routes.TicketData.replace(":id", id) as Routes)
  );
}
...
export async function CallAPI<T>(
  ...
  const result = (await Result.json()) as any;
  return {
